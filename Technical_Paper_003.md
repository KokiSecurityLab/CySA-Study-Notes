...
* **認証失敗時のセッション分離**: 認証に繰り返し失敗するノードを分離された処理経路へルーティングし、検証済みセッションに割り当てられたリソースを消費させないようにします（応答タイミング制御の詳細は　テクニカルペーパーシリーズ #013　を参照）。
...
3. **自動拒否とログ記録**: 非準拠のリクエストを境界で拒否し、関連するセキュリティイベントとの相関分析のためにログを記録します（SIEM連携の詳細は　テクニカルペーパーシリーズ #002　を参照）。
