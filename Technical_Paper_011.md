...
1.  **出所の検証：** 信号が主要プロセッサに受け入れられる前に、その出所を厳格に検証する。ソースに悪意ある「データ注入」の履歴がある場合、即座にファイアウォールの抵抗値を上昇させる。
2.  **構造的整合性分析：** 偽の情報には、例外なく内部ロジックに「ひび割れ」が存在する。これらのデータ構造を脳内で回転させることで（詳細は論文#014を参照）、システムはこれらの構造的欠陥を特定する。論理的精査の圧力によってデータが崩壊した場合、それは「虚偽」としてフラグが立てられる。
