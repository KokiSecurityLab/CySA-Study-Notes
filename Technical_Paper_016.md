...
* **ベースライン境界統制および行動統制との整合**: カーネル整合性の検知結果を　テクニカルペーパーシリーズ #001で定義されたベースライン境界統制、および テクニカルペーパーシリーズ #013 で定義された行動異常指標と突き合わせ、より広いセキュリティアーキテクチャ全体で検知を一貫させます。
...
3. **自動化された構成修復**: 逸脱が確認された場合、該当する構成は自動的に検証済みベースラインへ復元され、そのインシデントは関連するセキュリティイベントとの相関分析のために記録されます（ポストインシデントレビューの詳細は テクニカルペーパーシリーズ #006 を参照）。
