...
* **ベースライン境界統制との整合**: 暗号移行を テクニカルペーパーシリーズ #001 で定義したベースライン境界統制と突き合わせ、アルゴリズムの変更をより広いセキュリティアーキテクチャと一貫させます。
...
* **レガシー暗号化されたコールドストレージの隔離**: まだ再暗号化できていないデータは、テクニカルペーパーシリーズ #027 で定義した分類アプローチと整合する形でアクセス制限されたコールドストレージに隔離され、移行完了までの露出を低減します。
