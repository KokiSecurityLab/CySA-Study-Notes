...
* **ゼロトラストのセッション検証との整合**: ワークロードアイデンティティ検証を　テクニカルペーパーシリーズ #008で定義した継続的なリクエスト単位の検証アプローチと突き合わせ、ゼロトラストの原則をユーザーセッションからサービス間通信にまで拡張します。
...
* **デフォルト拒否型のサービス通信ポリシー**: サービスは、テクニカルペーパーシリーズ #001で定義したデフォルト拒否アプローチと整合する形で、明示的に許可された通信相手とのみ通信を許されます。
