...
* **暗号学的な構成フィンガープリント化**: 使用中のシステム指示、ガードレールポリシーセット、モデルバージョンをハッシュ化して検証可能なフィンガープリントを生成します。これは テクニカルペーパーシリーズ #001 で定義したベースラインハッシュのアプローチと整合します。
...
3. **不一致時の自動セッションリセット**: フィンガープリントがベースラインと一致しなくなったセッションは終了され、検証済み構成から再初期化されます。これは テクニカルペーパーシリーズ #016 で定義した自動修復アプローチと整合します。
...
* **構成変更経路の制限**: システム指示やガードレールポリシーの変更は、セッション内での修正ではなく、テクニカルペーパーシリーズ #021 で定義したものと同一のバージョン管理されたデプロイプロセスを通じてのみ適用されます。
